    ...
    <div class="card-header">
        <div class="row flex-between-end">
            <div class="col-auto align-self-center">
                <h5 class="mb-0" data-anchor="data-anchor" id="example">Edit Make</h5>
            </div>
        </div>
    </div>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="form-floating mb-3">
                        <input class="form-control" type="text" id="name" placeholder="Make Name" name="name"
                            value="{{ $make->name }}" />
                    ...
                    <div class="card shadow-md">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <h5 class="card-title mb-0">Make Image</h5>
                            ...
                            <div class="border border-dashed rounded d-flex align-items-center justify-content-center p-3"
                                ...
                                <img id="image-preview" class="{{ $make->image ? '' : 'd-none' }}"
                                    style="width: 100%; height: 100%; object-fit: contain;" alt="Image Preview"
                                    src="{{ $make->image ? asset($make->image) : '' }}" />
                                <div id="image-message" class="text-center text-muted {{ $make->image ? 'd-none' : '' }}">
                                    <i class="bi bi-image fs-1"></i>
                                    <p class="mt-2 small">Upload Make Image</p>
                                </div>
                            </div>
...
<script>
    function submitForm(event) {
        ...
        $.ajax({
                method: 'POST',
                url: '{{ route('make.update', $make->uid) }}',
                data: formData,
                processData: false,
                contentType: false,
                headers: {
