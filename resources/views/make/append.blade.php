...
@forelse ($data as $d)
    <tr class="btn-reveal-trigger">
        ...
        <td class="d-flex align-items-center gap-2">
            <div>
                <img src="{{ asset($d->image ?? 'assets/img/default-image.png') }}" alt="Make Image"
        ...
        <td class="text-end">
            <div class="dropdown font-sans-serif position-static">
                ...
                <div class="dropdown-menu dropdown-menu-end border py-0">
                    <div class="py-2">
                        <a class="dropdown-item" href="/make/{{ $d->uid }}/edit" target="_blank">
...
@empty
    <tr class="text-center">
        <td colspan="5">No Makes Found</td>
    </tr>
@endforelse
...
<script>
    $(document).ready(function() {
        ...
        $('.delete-btn').on('click', function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: `You want to ${isRestore ? 'restore' : 'delete'} this make?`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        error: function() {
                            showError(
                                `An error occurred while trying to ${isRestore ? 'restore' : 'delete'} the make.`
                                );
                        }
                    });
                }
        ...
        $('.status-btn').on('click', function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: `Do you want to ${currentStatus ? 'make Private' : 'make Public'} this make?`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#6b7280',
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (!response.error) {
                                ...
                                Swal.fire({
                                    title: 'Success!',
                                    text: `Make is now ${currentStatus ? 'Private' : 'Public'}.`,
                                    icon: 'success',
                                    timer: 1500,
                                    showConfirmButton: false
                                });
                        ...
                        error: function() {
                            showError('An error occurred while updating make status.');
                        }
                    });
                }
            });
