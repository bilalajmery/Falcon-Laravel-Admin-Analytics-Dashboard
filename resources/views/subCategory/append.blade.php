...
@forelse ($data as $d)
    <tr class="btn-reveal-trigger">
        ...
        <td class="d-flex align-items-center gap-2">
            <div>
                <img src="{{ asset($d->image ?? 'assets/img/default-image.png') }}" alt="Sub Category Image"
        ...
        <td class="text-end">
            <div class="dropdown font-sans-serif position-static">
                ...
                <div class="dropdown-menu dropdown-menu-end border py-0">
                    <div class="py-2">
                        <a class="dropdown-item" href="/subCategory/{{ $d->uid }}/edit" target="_blank">
...
<script>
    $(document).ready(function() {
        ...
        $('.delete-btn').on('click', function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: `You want to ${isRestore ? 'restore' : 'delete'} this sub category?`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        error: function() {
                            showError(
                                `An error occurred while trying to ${isRestore ? 'restore' : 'delete'} the sub category.`
                            );
                        }
                    });
                }
        ...
        $('.status-btn').on('click', function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: `Do you want to ${currentStatus ? 'make Private' : 'make Public'} this sub category?`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#6b7280',
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (!response.error) {
                                ...
                                Swal.fire({
                                    title: 'Success!',
                                    text: `Sub Category is now ${currentStatus ? 'Private' : 'Public'}.`,
                                    icon: 'success',
                                    timer: 1500,
                                    showConfirmButton: false
                                });
                        ...
                        error: function() {
                            showError('An error occurred while updating sub category status.');
                        }
                    });
                }
            });
