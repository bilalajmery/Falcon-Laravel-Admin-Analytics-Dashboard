...
<script>
    $(document).ready(function() {
        ...
        $('.delete-btn').on('click', function() {
            ...
            const successMessage = isRestore ? 'The employee has been restored.' : 'The employee has been deleted.';
            const errorMessage = isRestore ? 'Failed to restore the employee.' : 'Failed to delete the employee.';

            Swal.fire({
                title: 'Are you sure?',
                text: `You want to ${actionText} this employee?`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (!response.error) {
                                $row.fadeOut(300, function() {
                                    $(this).remove();
                                    getTableData(); // Refresh the employee list
                                });
                        ...
                        error: function() {
                            showError(`An error occurred while trying to ${actionText} the employee.`);
                        }
                    });
                }
            });
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (!response.error) {
                                ...
                                Swal.fire({
                                    title: 'Success!',
                                    text: `Employee is now ${isCurrentlyPublic ? 'Private' : 'Public'}.`,
                                    icon: 'success',
                                    timer: 1500,
                                    showConfirmButton: false
                                });
                                ...
                                showError(response.message || `Failed to update employee status.`);
                            }
                        },
                        error: function() {
