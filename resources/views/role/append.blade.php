        ...
        <td class="text-end">
            <div class="dropdown font-sans-serif position-static">
                ...
                <div class="dropdown-menu dropdown-menu-end border py-0">
                    <div class="py-2">
                        <a class="dropdown-item" href="{{ route('role.edit', $d->uid) }}" target="_blank">
                            <i class="fas fa-edit"></i>   Edit
                        </a>

                        <a class="dropdown-item" href="/role/permission/{{ $d->uid }}" target="_blank">
                            <i class="far fa-hand-paper"></i>   Permissions
                        </a>
...
<script>
    $(document).ready(function() {
        ...
        $('.delete-btn').on('click', function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: `You want to ${isRestore ? 'restore' : 'delete'} this role?`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        error: function() {
                            showError(
                                `An error occurred while trying to ${isRestore ? 'restore' : 'delete'} the role.`
                            );
                        }
                    });
                }
        ...
        $('.status-btn').on('click', function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: `Do you want to ${currentStatus ? 'make Private' : 'make Public'} this role?`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#6b7280',
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (!response.error) {
                                ...
                                Swal.fire({
                                    title: 'Success!',
                                    text: `Role is now ${currentStatus ? 'Private' : 'Public'}.`,
                                    icon: 'success',
                                    timer: 1500,
                                    showConfirmButton: false
                                });
                        ...
                        error: function() {
                            showError('An error occurred while updating role status.');
                        }
                    });
                }
            });
